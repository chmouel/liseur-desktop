...
| **M5 — Polished reader shell**        | Reader chrome, hidden reading mode, shortcuts, typography popover (font size, columns, page margins), progress footer, scrubber, full-screen, return to library                                                                                                                                                                                                                                                             | ✅ done |
...
| **M7 — Remote catalogs & sync**       | One server at a time, in order: **Komga → calibre-web → liseur-sync**. Connection settings, test connection, catalog sync, downloads, progress sync, conflict/catch-up UI. Capability-based interfaces; debounced/coalesced sync queue persisted across restarts; catalogs refresh when the window regains focus; reading sessions recorded locally and uploaded to liseur-sync so desktop reading counts in its statistics | ✅ done |

Later (unscheduled): reading statistics UI (the sessions behind it are already
recorded and uploaded; only the screen is missing), Markdown export of annotations,
